...
1. Starts the exact requested command once with `sudo -A` and a private askpass bridge.
2. If a cached credential or a command-specific `NOPASSWD` rule applies, sudo never starts the bridge helper. The command runs immediately and no password prompt appears.
...
5. The command receives `/dev/null` as stdin. Its exit code and stdout are returned without any password-dependent transformation. If the call completed without eliciting a password, stderr is returned too. After password elicitation, the whole stderr channel is suppressed because sudo authentication diagnostics and command stderr are not separable.
...
If a client does not declare form-elicitation support, `sudo-mcp` falls back to its original native `SUDO_ASKPASS` dialog. The exact authorization text is stored in a mode-`0600` file inside a private temporary directory. The helper verifies the bytes it reads against a server-provided SHA-256 digest and refuses to display modified content. Only the short path and fixed-size digest are placed in the helper environment. Because this path cannot reliably tell whether sudo actually invoked askpass, it suppresses stderr on every call:
...
- A successful authentication may update sudo's normal timestamp cache, so later calls during that window can run without another user prompt. `timestamp_timeout=0` and policies without reusable timestamp credentials are supported because authentication and execution use the same sudo process.
...
Restart Codex after changing the MCP list or approval policy. To force the first test to prompt rather than reuse a cached credential, run `sudo -k` yourself before invoking `sudo_run`. Never put the password in chat or in the tool arguments.
...
The result is a text block containing the exit code and stdout. Stderr is also included when the MCP path completes without eliciting a password. After MCP password elicitation, and for every native-fallback call, the stderr section contains a fixed suppression notice instead. Each returned output stream is truncated at 256 KiB. Output is never filtered based on the submitted password, so a command can still expose secrets through stdout or through stderr when stderr is returned.
...
- Command-specific `NOPASSWD` rules and cached credentials are honored before MCP elicitation, without retrying a command that may already have run.
...
- Output rendering never depends on the submitted password, avoiding a candidate-testing oracle. After MCP authentication, stdout is returned unchanged while the entire mixed sudo/command stderr channel is discarded and replaced with a fixed notice. Calls that require no elicitation return both streams normally; the native fallback always suppresses stderr.
